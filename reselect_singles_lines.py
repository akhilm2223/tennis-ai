"""
Reselect only the singles sidelines (left and right)
...
frame = None
display_frame = None
window_name = "Reselect Singles Lines"
scale_factor = 1.0

# Only these 2 lines
LINE_DEFINITIONS = [
    ('singles_left', 'Left Singles Sideline (2 points: top to bottom)', 2),
    ('singles_right', 'Right Singles Sideline (2 points: top to bottom)', 2),
]

def mouse_callback(event, x, y, flags, param):
    """Handle mouse clicks"""
    global temp_points, singles_lines, display_frame, scale_factor, current_line_index
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if current_line_index < len(LINE_DEFINITIONS):
            ...
            # Draw point
            cv2.circle(display_frame, (x, y), 10, (0, 255, 255), -1)  # Yellow
            ...
            if len(temp_points) == num_points:
                ...
                if current_line_index < len(LINE_DEFINITIONS):
                    ...
                    print(f"   Click {next_pts} points (TOP first, then BOTTOM)...")
                else:
                    print("\n🎉 Singles lines complete!")
                    print("   Press 's' to save")
            
            cv2.imshow(window_name, display_frame)

def reselect_singles(video_path, json_path, max_height=900):
    """Reselect only singles sidelines"""
    global singles_lines, temp_points, frame, display_frame, scale_factor, current_line_index
    
    print("=" * 70)
    print("🎾 RESELECT SINGLES SIDELINES ONLY")
    print("=" * 70)
    print()
    print("You will select 2 lines:")
    print("  1. Left Singles Sideline (vertical, near left side)")
    print("  2. Right Singles Sideline (vertical, near right side)")
    print()
    print("IMPORTANT: Click TOP point first, then BOTTOM point!")
    print("           This ensures vertical lines (same x-coordinate)")
    print()
    print("Controls:")
    ...
    # Draw existing lines on display (except singles)
    for line_name, line_data in existing_data['lines'].items():
        if line_name not in ['singles_left', 'singles_right']:
    ...
    # Start with first singles line
    line_name, line_desc, num_points = LINE_DEFINITIONS[0]
    print(f"\n📍 Line 1/2: {line_desc}")
    print(f"   Click {num_points} points (TOP first, then BOTTOM)...")
    print(f"   TIP: Try to keep same x-coordinate for perfectly vertical line!")
    ...
    while True:
        ...
        # Save
        if key == ord('s'):
            if len(singles_lines) == 2:
                # Update existing data with new singles lines
                for line_name, points in singles_lines.items():
                ...
                print(f"\n✅ Singles lines updated in: {json_path}")
                print("\n📊 New singles lines:")
                for name in singles_lines.keys():
                    print(f"   • {name}: {singles_lines[name][0]} → {singles_lines[name][1]}")
                
                cv2.destroyAllWindows()
                return existing_data
            else:
                print(f"⚠️  Need both singles lines")
        ...
        # Reset current line
            ...
            if len(temp_points) > 0:
                ...
                # Redraw completed singles lines
                for completed_name, completed_points in singles_lines.items():
                    ...
                    cv2.line(display_frame, pt1, pt2, (0, 255, 255), 4)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Reselect Singles Sidelines Only')
    ...
    result = reselect_singles(args.video, args.lines)
    
    if result:
        print("\n" + "="*70)
        print("✅ SUCCESS! Singles lines have been updated.")
        print("="*70)
        print("\nTo visualize:")
